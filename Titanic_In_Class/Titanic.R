...
gbmGrid <-  expand.grid(interaction.depth = c(1, 2, 3, 4), n.trees = seq(200, 10000, 200), shrinkage = c(.1,.05,.01,.005,.001) )

fit <- train(Survived ~ Pclass + Age + Sex + SibSp + Parch + Fare + Embarked, data = train, method = "gbm", trControl = fitControl, verbose=FALSE)
print(fit)
